...
Dec_before_use::Dec_before_use(Program_symbol_table &program_sym_table,
...
void Dec_before_use::dispatch(Array_dec &node)
{
    if (m_global_var_flag)
    {
        ...
        m_global_vars.push_back(std::pair(node.get_name(), node.m_array_size));
    }
    else if (m_func_args)
    {
...
void Dec_before_use::dispatch(Var_dec &node)
{
    ...
    if (m_global_var_flag)
    {
        ...
        m_global_vars.push_back(std::pair(node.get_name(), std::nullopt));
    }
    else if (m_func_args)
    {
        if (!m_prog_sym_table.add_register_var(node.get_name(), &node))
